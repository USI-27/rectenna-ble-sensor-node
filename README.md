...
### A Batteryless IoT Node Powered by a Pyramidal Rectenna WPT Front End
...
The board is organized into three functional blocks, each shown as a separate circuit in the Altium schematic.
...
| `C3` (220 µF) on `BAT`/`SUPERCAP` | Primary energy reservoir | A supercapacitor is used instead of a battery, since it tolerates the highly intermittent, low current charge profile typical of RF harvesting and avoids battery degradation or replacement, keeping the node fully maintenance free |
...
**Why this part:** The ANNA-B112 integrates a Bluetooth 5 radio, antenna, and Cortex M0 MCU in a single certified module, minimizing RF design risk and board area. That matters on a power constrained, harvesting driven node where every extra milliamp counts.
...
| `Y1` (32.768 kHz) | Real time clock crystal | Used for the low power sleep/RTC timer inside the BLE stack, allowing the MCU to stay in deep sleep between BLE advertising events. This is critical for surviving on a harvested microwatt to milliamp power budget |
| `C1`, `C2` (0.1 µF) | Decoupling | Local bypass on VCC and crystal load caps |
| `MOSI_BME` / `SCLK_BME` / `MISO_BME` / `CS_BME` | SPI bus to sensor | GPIO13–15 plus a dedicated chip select line route a full duplex SPI interface to the BME688 |
...
### 3.4 Full Schematic (PDF)

The complete Altium schematic, all three sub circuits above plus the board stack report, is available as a PDF:

📄 [**View the full schematic PDF**](docs/images/Nicla_BME688_ANNA_Project_IITrpr-1.pdf)

> GitHub renders PDFs inline when you click the link above (opens in the built in file viewer).
...
|---|---|
| ![Bare top](docs/images/1.png) | ![Bare bottom](docs/images/5.png) |
...
* **Keepout zone:** A dedicated no copper region is reserved beneath the ANNA-B112's integrated antenna to avoid detuning the BLE radio.
  ...
  ![Antenna keepout close-up](docs/images/2.png)
...
![Pyramidal rectenna 3D render](docs/images/6.png)
...
### 6.1 PCB photos — in place

| File | Shows |
|---|---|
| `docs/images/1.png` | Bare PCB, top |
| `docs/images/5.png` | Bare PCB, bottom |
| `docs/images/3.png` | Assembled board, top (used as the hero image above) |
